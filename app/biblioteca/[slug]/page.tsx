...
import { getGoogleDrivePublicUrl } from "@/lib/storage/google-drive";
  ...
  return (
    <main className="flex min-h-screen w-full flex-col gap-6 px-6 py-10 sm:px-10">
      ...
      <article className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-semibold text-zinc-900">Conteúdo</h2>
        {resource.contentMarkdown ? (
          <div className="mt-3 whitespace-pre-wrap text-sm leading-7 text-zinc-800">
            {resource.contentMarkdown}
          </div>
        ) : (
          <p className="mt-3 text-sm text-zinc-600">Este conteúdo ainda não possui texto interno.</p>
        )}
      </article>
              ...
              return (
                <li key={asset.id} className="rounded-xl border border-zinc-200 bg-zinc-50 p-3">
                  ...
                  {href ? (
                    ...
                    <p className="mt-2 text-xs text-amber-700">Mídia sem URL pública no momento.</p>
                  )}
                </li>
              );
            })}
